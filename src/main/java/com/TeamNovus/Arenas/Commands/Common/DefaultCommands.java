...
public class DefaultCommands {
	...
	public void helpCmd(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		int total = 0;
		sender.sendMessage(CommandManager.getExtra() + "__________________.[ " + CommandManager.getHighlight() + Arenas.getPlugin().getName() + CommandManager.getExtra() + " ].__________________");
		...
		for (int i = maxLines * page - maxLines; i < CommandManager.getCommands().size() && total < maxLines - 1; i++) {
			...
			if(!(command.hidden()) && com.TeamNovus.Arenas.Permission.has(command.permission(), sender)) {
